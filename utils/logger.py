...
class Logger:
    def __enter__(self):
        # Initialization or setup code can go here
        return self  # This should return the logger instance itself

    def __exit__(self, exc_type, exc_val, exc_tb):
        # Cleanup code can go here
        self.log_file.close()  # Ensure the log file is closed when the context is exited
    ...
    def visualize_rec(self, inp: Dict[str, torch.Tensor], out: Dict[str, torch.Tensor], name: str = None):
        # print("inp:", inp)
        # print("out:", out)
        # print("Input shape and type:", np.shape(inp), type(inp))
        # print("Output shape and type:", np.shape(out), type(out))
        if name:
        ...
        viz_params = {}
        if inp is not None:
            viz_params.update(inp)
        if out is not None:
            viz_params.update(out)

    # Check if the visualizer has the 'visualize' method
        print(dir(self.visualizer))  # This will print all attributes and methods of the visualizer instance
        if hasattr(self.visualizer, 'visualize'):
            # print("Visualization parameters:", viz_params)
            image = self.visualizer.visualize(**viz_params)
        else:
            raise AttributeError("The Visualizer object does not have a 'visualize' method")
    ...
    def log_epoch(self, epoch, models=None, inp=None, out=None, name=None):
        ...
        if self.names:
            ...
            print("Warning: Loss names are not initialized.")
        image = self.visualize_rec(inp, out, name=name)
        return image, self.epoch_losses if self.names else {}
    
from torchvision.utils import flow_to_image

class Visualizer:
    def __init__(self, kp_size: int=5, draw_border:bool=False, 
                 colormap: str='gist_rainbow',row_size:int=4):
